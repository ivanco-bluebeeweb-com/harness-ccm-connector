"""Connection lifecycle for Harness CCM Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from harness_ccm_client import HarnessCcmClient
...
_SECRET = "harness_ccm_connections"
...
async def connect_harness_ccm(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Harness CCM Connector."""
    ...
    record = {
        "id": cid,
        "label": params.label or "Harness CCM Account",
...
@chat.function(
    "list_connections",
    "List connected Harness CCM accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
async def disconnect_harness_ccm(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
