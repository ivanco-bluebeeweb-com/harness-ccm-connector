...
async def connect_harness_ccm_connector(ctx, params: ConnectParams) -> ActionResult:
...
async def disconnect_harness_ccm_connector(ctx, params: ConnectionIdParams) -> ActionResult:
    conns = await _load_conns(ctx)
    if not conns:
