...
async def connect_harness_ccm_connector(params: ConnectParams, ctx) -> ActionResult:
    ...
    return ActionResult.ok(rec, summary=f"Connected Harness CCM ({rec['label']}).")
...
async def disconnect_harness_ccm_connector(params: ConnectionIdParams, ctx) -> ActionResult:
    ...
    if params.connection_id:
        ...
        conns.clear()
    await _save_conns(ctx, conns)
    return ActionResult.ok({"success": True, "message": "Disconnected successfully."}, summary="Disconnected Harness CCM connection.")
