"""HTTP client for Harness CCM API."""
from __future__ import annotations
import httpx
from typing import Any, Optional

DEFAULT_BASE = "https://app.harness.io/gateway/ccm/api"
...
class HarnessCCMClient:
    def __init__(self, api_key: str, base_url: str = ""):
        ...
        self.headers = {
            ...
            "User-Agent": "Imperal-HarnessCCM-Connector/1.0.0"
        }
    ...
    async def verify_auth(self) -> dict[str, Any]:
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            try:
                resp = await client.get(f"{self.base_url}/costs", headers=self.headers)
    ...
    async def list_costs(self, limit: int = 20) -> list[dict[str, Any]]:
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            resp = await client.get(f"{self.base_url}/costs", headers=self.headers, params={"limit": limit})
            if resp.status_code == 200:
                data = resp.json()
                if isinstance(data, list): return data
                for k in ["data", "costs", "items", "results"]:
    ...
    async def get_costrecord(self, costrecord_id: str) -> dict[str, Any]:
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            resp = await client.get(f"{self.base_url}/costs/{costrecord_id}", headers=self.headers)
            if resp.status_code == 200:
                return resp.json()
